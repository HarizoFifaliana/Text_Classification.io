...
class LSTMModel(nn.Module):
    ...
    def forward(self, x):
        ...
        last_hidden_state = lstm_out[:, -1, :]  # Dernière sortie LSTM
...
def main():
    # Charger et pré-traiter les données IMDb
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=64, shuffle=True, collate_fn=lambda x: pad_sequence([i[0] for i in x], batch_first=True, padding_value=0))
    test_dataloader = DataLoader(test_dataset, batch_size=64, shuffle=False, collate_fn=lambda x: pad_sequence([i[0] for i in x], batch_first=True, padding_value=0))
    ...
    model = LSTMModel(vocab_size, embedding_dim, hidden_dim, num_layers, max_len).to(device)
